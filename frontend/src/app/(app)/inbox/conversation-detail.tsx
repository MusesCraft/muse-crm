...
function QuickRepliesPanel({ onSelect, onClose }: { onSelect: (text: string) => void; onClose: () => void }) {
  ...
  const categories = Array.from(new Set(mockQuickReplies.map((r) => r.category)));
    ...
    <div className="absolute bottom-full left-0 mb-2 w-80 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-lg z-50 max-h-80 flex flex-col">
      ...
      <div className="px-3 pb-2">
        <input
          ...
          placeholder="搜尋語錄..."
          className="w-full text-xs bg-zinc-50 dark:bg-zinc-700 border border-zinc-200 dark:border-zinc-600 rounded-lg px-3 py-1.5 text-zinc-700 dark:text-zinc-200 placeholder:text-zinc-400 dark:placeholder:text-zinc-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
        />
      </div>
      {/* List */}
...
function AiSuggestionCard({
  ...
  return (
    <div className="mx-4 mb-2 px-3 py-2.5 rounded-xl bg-gradient-to-r from-purple-500/5 to-blue-500/5 dark:from-purple-500/10 dark:to-blue-500/10 border border-purple-500/20 dark:border-purple-500/30">
...
export function ConversationDetail({ conversationId, onClose }: ConversationDetailProps) {
  ...
  const handleSend = useCallback(() => {
    ...
    // TODO: POST /api/v1/inbox/conversations/:id/messages
      ...
      <div className="border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex-shrink-0">
        ...
        <div className="px-4 py-3 flex items-end gap-2">
          ...
          <textarea
            ...
            placeholder="輸入訊息..."
            rows={1}
            className="flex-1 bg-zinc-50 dark:bg-zinc-800 rounded-xl px-4 py-2 text-sm text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 dark:placeholder:text-zinc-500 border border-zinc-200 dark:border-zinc-700 focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none max-h-[120px]"
          />
