...
function SentimentBadge({ sentiment }: { sentiment: string }) {
  const config: Record<string, { icon: typeof Smile; color: string; label: string }> = {
    positive: { icon: Smile, color: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/25', label: '正面' },
    neutral: { icon: Meh, color: 'bg-zinc-500/15 text-zinc-400 border-zinc-500/25', label: '中性' },
    negative: { icon: Frown, color: 'bg-red-500/15 text-red-400 border-red-500/25', label: '負面' },
  };
...
function UrgencyBadge({ urgency }: { urgency: string }) {
  const config: Record<string, { color: string; label: string }> = {
    high: { color: 'bg-red-500/15 text-red-400 border-red-500/25', label: '高' },
    medium: { color: 'bg-amber-500/15 text-amber-400 border-amber-500/25', label: '中' },
    low: { color: 'bg-zinc-500/15 text-zinc-400 border-zinc-500/25', label: '低' },
  };
    ...
    <div className="space-y-3">
      {demandSummary && (
        <div>
          ...
          <p className="text-xs text-zinc-300 leading-relaxed">{demandSummary}</p>
        </div>
      )}
      ...
      {suggestedAction && (
        <div>
          ...
          <p className="text-xs text-emerald-400/90 bg-emerald-500/5 border border-emerald-500/10 rounded-md px-2.5 py-1.5">
            {suggestedAction}
          </p>
        </div>
      )}
...
export function CustomerSidebar({
  ...
  if (collapsed) {
    return (
      <div className="w-10 bg-zinc-900 border-l border-zinc-800 flex flex-col items-center pt-3 transition-all duration-200">
        <button
  ...
  if (loading || !contact) {
    return (
      <div className="w-80 bg-zinc-900 border-l border-zinc-800 flex items-center justify-center transition-all duration-200">
        <LoadingSpinner />
      </div>
    );
  }
  ...
  return (
    <div className="w-80 bg-zinc-900 border-l border-zinc-800 flex flex-col overflow-hidden transition-all duration-200">
      {/* ── Header ── */}
      <div className="p-4 border-b border-zinc-800 flex items-center gap-3">
        ...
        <button
          ...
          className="p-1.5 rounded-md text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800 transition-colors flex-shrink-0"
          title="收合"
        >
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="p-4 border-b border-zinc-800/50">
          <h4 className="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-3">基本資訊</h4>
          <div className="space-y-2.5">
            {entityInfo.phone && (
              <div className="flex items-center gap-2.5 text-sm">
                <Phone className="w-3.5 h-3.5 text-zinc-500 flex-shrink-0" />
                <span className="text-zinc-300">{entityInfo.phone}</span>
              </div>
            )}
            {entityInfo.address && (
              <div className="flex items-center gap-2.5 text-sm">
                <MapPin className="w-3.5 h-3.5 text-zinc-500 flex-shrink-0" />
                <span className="text-zinc-300">{entityInfo.address}</span>
              </div>
            )}
            {entityInfo.email && (
              <div className="flex items-center gap-2.5 text-sm">
                <Mail className="w-3.5 h-3.5 text-zinc-500 flex-shrink-0" />
                <span className="text-zinc-300">{entityInfo.email}</span>
              </div>
            )}
            {entityInfo.company && (
              <div className="flex items-center gap-2.5 text-sm">
                <Building2 className="w-3.5 h-3.5 text-zinc-500 flex-shrink-0" />
                <span className="text-zinc-300">{entityInfo.company}</span>
              </div>
            )}
            {contact.display_name && contact.display_name !== contact.name && (
              <div className="flex items-center gap-2.5 text-sm">
                <Building2 className="w-3.5 h-3.5 text-zinc-500 flex-shrink-0" />
                <span className="text-zinc-300 text-xs">{contact.display_name}</span>
              </div>
            )}
            <div className="flex items-center gap-2.5 text-sm">
        ...
        <div className="p-4 border-b border-zinc-800/50">
          ...
          <div className="flex flex-wrap gap-1.5">
            {contact.tags && contact.tags.length > 0 ? (
              ...
              <span className="text-xs text-zinc-600">無標籤</span>
            )}
          </div>
        </div>
            ...
            <div className="space-y-1.5">
              {otherConversations.map((conv) => (
                <button
                  ...
                  className="w-full flex items-center justify-between p-2 rounded-md bg-zinc-800/30 hover:bg-zinc-800/60 transition-colors text-left"
                >
          ...
          {recentNotes.length > 0 ? (
            <div className="space-y-2">
              {recentNotes.map((note) => (
                <div
                  key={note.id}
                  className="p-2.5 rounded-md bg-zinc-800/30 border-l-2 border-yellow-500/30"
                >
            ...
            <p className="text-xs text-zinc-600">尚無備註</p>
          )}
        </div>
      </div>
    </div>
