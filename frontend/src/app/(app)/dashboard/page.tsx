...
function ChannelBar({
  ...
  return (
    <div className="bg-zinc-950/50 border border-zinc-800 rounded-lg p-5">
      <h3 className="text-sm font-semibold text-zinc-200 mb-4">渠道分布</h3>
      ...
      <div className="h-4 rounded-full overflow-hidden flex bg-zinc-800 mb-4">
        {distributions.map((d) => (
          <div
            key={d.channel}
            className={cn('h-full transition-all', colors[d.channel] || 'bg-zinc-600')}
      ...
      <div className="flex flex-wrap gap-4">
        {distributions.map((d) => (
          <div key={d.channel} className="flex items-center gap-2">
            <div className={cn('w-3 h-3 rounded-full', colors[d.channel] || 'bg-zinc-600')} />
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-zinc-400 mt-1">系統總覽與統計數據</p>
      </div>
